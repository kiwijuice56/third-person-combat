...
const TARGETING_OFFSET: Vector2 = Vector2(2.0, 2.0)
...
func recenter() -> void:
	...
	tween.tween_property(self, "transform:basis", new_basis, TRANSITION_TIME)
	tween.tween_property(self, "position", Vector3(plane_direction.x * NORMAL_OFFSET.x, NORMAL_OFFSET.y, plane_direction.y * NORMAL_OFFSET.x), TRANSITION_TIME)
...
func look_at_target(target: Node3D) -> void:
	...
	var tween: Tween = get_tree().create_tween().parallel()
	tween.tween_property(self, "transform:basis", new_basis, TRANSITION_TIME)
	tween.tween_property(self, "position", Vector3(target_to_player.x * TARGETING_OFFSET.x, TARGETING_OFFSET.y, target_to_player.z * TARGETING_OFFSET.x), TRANSITION_TIME)
